...
async function handleSearch(request, env, ctx) {
  ...
  let payload; try { payload = await response.json(); } catch { return response; }
  if (!payload?.ok) return json(payload, response.status);
  const candidates = collectCandidatePages(payload).slice(0, MAX_HEALTH_PAGES);
  if (!candidates.length) { payload.curatorContext = payload.curatorContext || {}; payload.curatorContext.siteHealth = { configured: true, ok: true, mode: 'on-demand-batch', checkedPageCount: 0, error: null }; return json(payload, response.status); }
  ...
  if (health.ok) { const byPage = normalizeHealth(health.data); payload.recommendations = enrichRecommendations(payload.recommendations || [], byPage); payload.today = enrichRecommendations(payload.today || [], byPage); }
...
async function fetchHealthBatch(baseUrl, pages) { const endpoint = `${String(baseUrl).replace(/\/$/, '')}/check`; const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), 12000); try { const response = await fetch(endpoint, { method: 'POST', headers: { 'content-type': 'application/json', accept: 'application/json', 'user-agent': 'CuratorOS-Search-Intelligence/1.1' }, body: JSON.stringify({ pages }), signal: controller.signal }); const text = await response.text(); let data = null; try { data = text ? JSON.parse(text) : null; } catch { throw new Error(`site-health returned non-JSON HTTP ${response.status}: ${text.slice(0, 180)}`); } if (!response.ok || data?.ok === false) throw new Error(data?.error || `site-health returned HTTP ${response.status}`); return { ok: true, data }; } catch (error) { return { ok: false, data: null, error: error?.name === 'AbortError' ? 'site-health batch timed out' : (error?.message || String(error)) }; } finally { clearTimeout(timer); } }
